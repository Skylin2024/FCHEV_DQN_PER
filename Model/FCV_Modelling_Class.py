...
class FCV_model():
    ...
    def __init__(self):
        ...
        self.fc_pwr_min, self.fc_pwr_max = 500, 50*1000
        ...
        ## Battery parameters 使用的是26Ah，8kWh的动力电池
        Num_cell = 25
    ...
    def run(self, veh_spd, veh_acc, ess_soc, fc_pwr, fc_pwr_pre, output_soc_f_only=False, dt=1.0):
        ## Longitudinal force balance equation
        # Wheel speed (rad/s)
        w_whl = veh_spd / self.veh_whl_radius
        # Wheel torque (Nm)
        ...
        ## Calculate motor speed, torque and efficiency
        mc_spd = w_whl * self.veh_fd_ratio
        mc_trq = T_whl / self.veh_fd_ratio
        mc_eff = (mc_spd == 0) + (mc_spd != 0) * self.mc_eff_func(mc_trq, mc_spd)
        inf_mc = (np.isnan(mc_eff)) + (mc_trq < 0) * (mc_trq < self.mc_max_gen_trq_func(mc_spd)) + (mc_trq >= 0) * (mc_trq > self.mc_max_trq_func(mc_spd))

        mc_outpwr = mc_trq * mc_spd
        mc_inpwr = mc_outpwr * ((mc_eff)**(-np.sign((mc_outpwr))))
        ...
        if ess_voc ** 2 >= 4 * ess_r_int * ess_pwr:
            ess_cur = ess_eff * (ess_voc - np.sqrt(ess_voc ** 2 - 4 * ess_r_int * ess_pwr)) / (2*ess_r_int)
            # done = False
        else:
            # print('动力电池功率不足以支撑动力系统运行')
            ess_cur = ess_eff * ess_voc / (2 * ess_r_int)
            # done = True
        ...
        # 计算燃料电池衰减的等效氢耗
        Mfc = 100 * 50  # 每kW燃料电池系统的造价
        Ch2 = 4.4  # 每kg氢气价格
        fc_pwr = fc_pwr / 1000
        fc_pwr_pre = fc_pwr_pre / 1000
        # start - stop
        if fc_pwr > 0 and fc_pwr == 0:
            deg_ss = 13.79e-6
        else:
            deg_ss = 0
        # high power
        if fc_pwr > 45:
            deg_high = 2.778e-3 * 1e-6
        else:
            deg_high = 0
        # low power
        if fc_pwr > 0 and fc_pwr <= 2.5:
            deg_low = 2.406e-3 * 1e-6
        else:
            deg_low = 0

        # power change
        if abs(fc_pwr - fc_pwr_pre) > 8:
            deg_chg = 0.4185e-6 * np.abs(fc_pwr - fc_pwr_pre)
        else:
            deg_chg = 0

        Percentage_FC = deg_chg + deg_low + deg_high + deg_ss  # 计算出来的为百分数
        FcDegSum = Mfc * Percentage_FC / (0.1 * Ch2)
        out['Percentage_FC'] = Percentage_FC
        out['FcDegSum'] = FcDegSum   # 单位为g

        # 计算动力电池衰减的等效氢耗
        C_Bat = 150  # dollar per KWh
        CH2 = 4.4  # dollar per kg
        z = 0.824
        B = 0.0032
        Ea = 15162
        R = 8.314
        A = 1156
        max_Q = self.ess_Q /3600
        Q = max_Q
        C_Rate = abs(ess_cur) / Q
        T = 25
        Q_loss = z * B ** (1 / z) / 3600 * np.exp((-Ea + A * C_Rate) /
                 (z * R * (273.15 + T))) * (Q ** ((z - 1) / z)) * abs(ess_cur)

        mbat_deg = C_Bat * max_Q * 320 / 1000 / CH2 * Q_loss / 0.2
        out['mbat_deg'] = mbat_deg * 1000  # 单位为g
        out['Percentage_Bat'] = Q_loss
        # out['done'] = done
